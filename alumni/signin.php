...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($stmt->num_rows == 1) {
        ...
        if (password_verify($password, $hashedPassword)) {
            // Direct login without OTP: fetch user email and onboarded flag
            $userStmt = $conn->prepare("SELECT email, onboarded FROM users WHERE user_id = ?");
            $userStmt->bind_param('i', $userId);
            $userStmt->execute();
            $userStmt->bind_result($userEmail, $onboarded);
            $userStmt->fetch();
            $userStmt->close();
            ...
            if ($isAjax) {
                $redirect = ((int)$onboarded === 0) ? 'UpdateAccount.php' : 'index.php';
...
<div class="signup-one-container">
  ...
  <div class="right-panel" style="order: 1;">
    <form id="signInForm" class="signup-one-form" method="post" action="">
      ...
      <p class="signin-link"><a href="forgot-password.php">Forgot your password?</a></p>
