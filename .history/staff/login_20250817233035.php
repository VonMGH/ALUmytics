        ...
        if (!isset($toastData)) { // Only proceed if no error so far
            if (password_verify($password, $hashedPassword)) {
                if ($emailVerified == 0) {
                    ...
                    if ($isAjax) {
                        header('Content-Type: application/json');
                        echo json_encode(['success' => true, 'redirect' => '../alumni/email-verification.php']);
                        exit;
                    }
                    // Redirect instantly for email verification
                    header('Location: ../alumni/email-verification.php');
                    exit;
                } else {
                    $_SESSION['user_id'] = $userId;
                    $_SESSION['email'] = $email;
