...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_POST['cert_name']) && is_array($_POST['cert_name'])) {
        ...
        $certCategories = isset($_POST['cert_category']) && is_array($_POST['cert_category']) ? $_POST['cert_category'] : [];
        $certIssuers = isset($_POST['cert_issuer']) && is_array($_POST['cert_issuer']) ? $_POST['cert_issuer'] : [];
        ...
        foreach ($certNames as $index => $name) {
            ...
            $categoryVal = isset($certCategories[$index]) ? $certCategories[$index] : '';
            $issuerVal = isset($certIssuers[$index]) ? trim($certIssuers[$index]) : '';
            ...
            if (isset($_FILES['cert_file']['tmp_name'][$index]) && is_uploaded_file($_FILES['cert_file']['tmp_name'][$index])) {
                ...
                $ext = strtolower(pathinfo($certOriginalName, PATHINFO_EXTENSION));
                $allowed = ['pdf','jpg','jpeg','png'];
                if (in_array($ext, $allowed)) {
                    $certUniqueName = uniqid('cert_') . "_" . $certOriginalName;
                    $fullPath = $certTargetDir . $certUniqueName;
                    $filePathSaved = "uploads/certification/" . $certUniqueName; // Save relative path
                    move_uploaded_file($certTmpPath, $fullPath);
                }
            }
            
            // Insert with the fields that Acertification.php expects
            $stmt = $conn->prepare("INSERT INTO certifications (user_id, certification_name, category, issuing_body, certification_date, certification_file, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())");
            $stmt->bind_param("isssss", $user_id, $name, $categoryVal, $issuerVal, $dateVal, $filePathSaved);
            $stmt->execute();
            $stmt->close();
            
            // Debug: Log the file path being saved
            error_log("Certification file saved: " . ($filePathSaved ?? 'NULL'));
        }
    }
    ...
    if (!empty($_POST['award_name']) && is_array($_POST['award_name'])) {
        ...
        $awardedBy = isset($_POST['awarded_by']) && is_array($_POST['awarded_by']) ? $_POST['awarded_by'] : [];
        $awardYears = isset($_POST['award_year']) && is_array($_POST['award_year']) ? $_POST['award_year'] : [];
        $awardDates = isset($_POST['award_date']) && is_array($_POST['award_date']) ? $_POST['award_date'] : [];
        $awardDescriptions = isset($_POST['award_description']) && is_array($_POST['award_description']) ? $_POST['award_description'] : [];
        ...
        foreach ($awardNames as $index => $name) {
            ...
            $awardedByVal = isset($awardedBy[$index]) ? trim($awardedBy[$index]) : '';
            $yearVal = isset($awardYears[$index]) ? $awardYears[$index] : '';
            $dateVal = isset($awardDates[$index]) ? $awardDates[$index] : null;
            $descriptionVal = isset($awardDescriptions[$index]) ? trim($awardDescriptions[$index]) : '';
            ...
            if (isset($_FILES['award_file']['tmp_name'][$index]) && is_uploaded_file($_FILES['award_file']['tmp_name'][$index])) {
                ...
                $ext = strtolower(pathinfo($awardOriginalName, PATHINFO_EXTENSION));
                $allowed = ['pdf','jpg','jpeg','png'];
                if (in_array($ext, $allowed)) {
                    $awardUniqueName = uniqid('award_') . "_" . $awardOriginalName;
                    $fullPath = $awardTargetDir . $awardUniqueName;
                    $filePathSaved = "uploads/awards/" . $awardUniqueName; // Save relative path
                    move_uploaded_file($awardTmpPath, $fullPath);
                }
            }
            
            // Insert using award_title field to match Aawards.php
            $stmt = $conn->prepare("INSERT INTO awards (user_id, award_title, category, awarded_by, award_year, award_date, description, award_file, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())");
            $stmt->bind_param("isssssss", $user_id, $name, $categoryVal, $awardedByVal, $yearVal, $dateVal, $descriptionVal, $filePathSaved);
            $stmt->execute();
            $stmt->close();
            
            // Debug: Log the file path being saved
            error_log("Award file saved: " . ($filePathSaved ?? 'NULL'));
        }
    }
...
<form class="signup-form" method="post" action="" enctype="multipart/form-data">
    ...
    <div class="file-actions" style="margin-top: 10px; text-align: center;">
        <button type="button" class="view-btn" onclick="openImageModal('<?php echo htmlspecialchars($personal['profile_photo']); ?>', 'Profile Photo')" style="margin-right: 5px; padding: 5px 10px; background: #007bff; color: white; border: none; border-radius: 3px; cursor: pointer;">
            <span class="icon">👁</span> View
        </button>
        <a href="<?php echo htmlspecialchars($personal['profile_photo']); ?>" download class="download-btn" style="margin-left: 5px; padding: 5px 10px; background: #28a745; color: white; text-decoration: none; border-radius: 3px;">
...
<div class="certification-section">
    ...
    <div class="certification-entry">
        <input type="text" id="certification-name" name="cert_name[]" placeholder="Certification Name">
        ...
        <input type="text" id="cert-issuer" name="cert_issuer[]" placeholder="Issuing Organization">
        <input type="date" id="issue-date" name="cert_date[]" placeholder="Issue Date">
    </div>

    <!-- Upload Section for Certifications -->
    <div class="upload-section">
        <label for="upload-cert">Upload Certification</label>
        <input type="file" id="upload-cert" name="cert_file[]" accept=".pdf,image/*" onchange="previewFile(this, 'cert-preview-0')">
        <div id="cert-preview-0" class="file-preview-container" style="display: none; margin-top: 10px;"></div>
    </div>
...
<div class="awards-section">
    ...
    <div class="upload-section">
        ...
        <input type="file" id="upload-award" name="award_file[]" accept=".pdf,image/*" onchange="previewFile(this, 'award-preview-0')">
        <div id="award-preview-0" class="file-preview-container" style="display: none; margin-top: 10px;"></div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (addAwardBtn && awardContainer) {
    ...
    addAwardBtn.addEventListener('click', function(){
      ...
      wrapper.innerHTML = `
        <input type="text" name="award_name[]" placeholder="Award Title">
        <select name="award_category[]" class="award-category-select">
          <option value="" disabled selected>Select Category</option>
          <option value="Academic Excellence">Academic Excellence</option>
          <option value="Leadership">Leadership</option>
        ...
        <input type="text" name="awarded_by[]" placeholder="Awarded By (Organization)">
        <input type="number" name="award_year[]" placeholder="Year" min="1900" max="2099">
        <input type="date" name="award_date[]" placeholder="Date Received">
        <textarea name="award_description[]" placeholder="Description (optional)" rows="2"></textarea>
        <div class="upload-section">
          <label>Upload Award Certificate</label>
          <input type="file" id="award-file-${awardCounter}" name="award_file[]" accept=".pdf,image/*" onchange="previewFile(this, 'award-preview-${awardCounter}')">
          <div id="award-preview-${awardCounter}" class="file-preview-container" style="display: none; margin-top: 10px;"></div>
        </div>
      `;
...
<div id="imageModal" class="image-modal" style="display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.9);">
  <div class="image-modal-content" style="margin: auto; display: block; width: 80%; max-width: 700px; margin-top: 50px;">
    ...
    <div style="text-align: center; margin-top: 20px;">
      <a id="downloadImageLink" download class="download-btn" style="padding: 10px 20px; background: #28a745; color: white; text-decoration: none; border-radius: 5px;">
        Download Image
      </a>
    </div>
  </div>
