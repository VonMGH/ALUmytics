...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['profile_photo']) && $_FILES['profile_photo']['error'] == 0) {
        ...
        if (in_array($file_type, $allowed_types) && $file_size <= 2 * 1024 * 1024) {
            ...
            if (move_uploaded_file($file_tmp, $new_profile_photo)) {
                // Successfully uploaded
                error_log("Profile photo uploaded successfully: " . $new_profile_photo);
            } else {
                error_log("Failed to move uploaded file to: " . $new_profile_photo);
                echo "<div class='alert alert-danger'>Failed to upload profile photo. Please try again.</div>";
                exit;
            }
        } else {
            echo "<div class='alert alert-danger'>Invalid file type or size. Please upload JPEG/PNG under 2MB.</div>";
            exit;
        }
    }
