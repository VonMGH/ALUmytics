...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $stmt = $conn->prepare("SELECT user_id, email_verified, full_name FROM users WHERE email = ?");
    $stmt->bind_param('s', $email);
    $stmt->execute();
    $stmt->store_result();
    if ($stmt->num_rows > 0) {
        $stmt->bind_result($existing_user_id, $existing_verified, $existing_full_name);
        $stmt->fetch();
        if ($existing_verified == 0) {
            ...
            $update->bind_param('ssi', $otp, $otpExpiresAt, $existing_user_id);
            $update->execute();
            $update->close();
            sendVerificationOTP($email, $existing_full_name, $otp);
            $_SESSION['registration_email'] = $email;
            $_SESSION['otp_user_id'] = $existing_user_id;
            if ($isAjax) {
                header('Content-Type: application/json');
                echo json_encode(['success' => true, 'redirect' => 'email-verification.php']);
                exit;
            } else {
                echo "<script>alert('Account exists but is not verified. Please check your email for the verification code.'); window.location.href = 'email-verification.php';</script>";
                exit;
            }
        } else {
    ...
    if (!empty($errors)) {
        ...
        // Insert new user into the database (ensure unverified and not onboarded by default)
        $stmt = $conn->prepare("INSERT INTO users (full_name, email, password_hash, otp_code, otp_expires_at, email_verified, onboarded, created_at) VALUES (?, ?, ?, ?, ?, 0, 0, ?)");
        $stmt->bind_param('ssssss', $full_name, $email, $hashedPassword, $otp, $otpExpiresAt, $createdAt);
        if ($stmt->execute()) {
            ...
            $stmt_personal->close();
            // Send OTP email
            $emailSent = sendVerificationOTP($email, $full_name, $otp);
            $_SESSION['registration_email'] = $email;
            ...
            if ($emailSent) {
                ...
                echo "<script>alert('Registration successful! Please check your email for the verification code.'); window.location.href = 'email-verification.php';</script>";
            } else {
                if ($isAjax) {
                    header('Content-Type: application/json');
                    echo json_encode(['success' => false, 'error' => 'Registration successful, but there was an issue sending the verification code. Please contact support.']);
                    exit;
                }
                echo "<script>alert('Registration successful, but there was an issue sending the verification code. Please contact support.'); window.location.href = 'email-verification.php';</script>";
            }
        } else {
            if ($isAjax) {
                header('Content-Type: application/json');
