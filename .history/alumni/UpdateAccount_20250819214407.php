...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_POST['award_name']) && is_array($_POST['award_name'])) {
        ...
        foreach ($awardNames as $index => $name) {
            ...
            // Insert using both award_name and award_title for compatibility
            $stmt = $conn->prepare("INSERT INTO awards (user_id, award_name, award_title, category, awarded_by, award_year, award_date, description, award_file, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
            $stmt->bind_param("issssisss", $user_id, $name, $name, $categoryVal, $awardedByVal, $yearVal, $dateVal, $descriptionVal, $filePathSaved);
            $stmt->execute();
            $stmt->close();
        }
    }
    ...
    addCertBtn.addEventListener('click', function() {
      ...
      wrapper.innerHTML = `
        ...
        <div class="upload-section">
          ...
          <input type="file" name="cert_file[]" accept=".pdf,image/*" onchange="previewFile(this, 'cert-preview-${certCounter}')">
    ...
    addAwardBtn.addEventListener('click', function(){
      ...
      wrapper.innerHTML = `
        ...
        <div class="upload-section">
          ...
          <input type="file" name="award_file[]" accept=".pdf,image/*" onchange="previewFile(this, 'award-preview-${awardCounter}')">
